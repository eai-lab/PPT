...
def import_ssl_finetune_lightmodule(cfg: DictConfig, log):
    """
    ...
    """
    if cfg.light_model.light_name == "light_patchtst_fullfinetune":
        from ssl_light_model.light_patchtst_fullfinetune import LitModel
    elif cfg.light_model.light_name == "light_pits_fullfinetune":
        from ssl_light_model.light_pits_fullfinetune import LitModel
    elif cfg.light_model.light_name == "light_fullfinetune_with_ppt":
        from ssl_light_model.light_fullfinetune_with_ppt import LitModel
    else:
...
def collect_permute_test_sets_dir(cfg, use_all_test_sets=False):
    ...
    all_test_set_dirs = glob.glob(f"data/{permute_sets_dir}/" + "random_*")
...
def collect_test_sets_dir(cfg, test_dir, use_all_test_sets=False):
    test_patch_size_list = cfg.permute_test.patch_size_sets
    data_dirs = []
    all_test_set_dirs = glob.glob(f"data/{test_dir}/" + "*.pkl")
    if use_all_test_sets:
        return all_test_set_dirs
    else:
        for test_set_dir in all_test_set_dirs:
...
def collect_predict_epoch_end_supervised(cfg, trainer, outputs, data_name):
    ...
    # pretrain model path
    test_label["load_pt_path"] = cfg.load_pt_path 
    test_label["pretrain_pt_file"] = cfg.pretrain_pt_file
